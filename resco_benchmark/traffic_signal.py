        ...
        for j in range(0, len(phases)):
            if i != j:
                ...
                for sig_idx in range(len(phases[i].state)):
                    if (
                        phases[i].state[sig_idx] == "G"
                        or phases[i].state[sig_idx] == "g"
                    ) and (
                        phases[j].state[sig_idx] == "r"
                        or phases[j].state[sig_idx] == "s"
                    ):
                        need_yellow = True
                        yellow_str += "y"
                    else:
                ...
                if need_yellow:  # If a yellow is required
                    ...
                    yellow_dict[str(i) + "_" + str(j)] = (
                        len(new_phases) - 1
                    )  # The index of the yellow phase in SUMO
...
class Signal:
    def __init__(self, map_name, sumo, id, yellow_length, phases):
            ...
            for direction in self.downstream:
                ...
                if dwn_signal is not None:  # A downstream intersection exists
                    ...
                    for key in dwn_lane_sets:  # Find all inbound lanes from upstream
                        if (
                            key.split("-")[0] == direction
                        ):  # Downstream direction matches
        ...
        self.waiting_times = (
            dict()
        )  # SUMO's WaitingTime and AccumulatedWaiting are both wrong for multiple signals
    ...
    def observe(self, step_length, distance):
        ...
        for lane in self.lanes:
            ...
            for vehicle in lane_vehicles:
                ...
                vehicle_measures["wait"] = (
                    self.waiting_times[vehicle] if vehicle in self.waiting_times else 0
                )
                vehicle_measures["speed"] = self.sumo.vehicle.getSpeed(vehicle)
                vehicle_measures["acceleration"] = self.sumo.vehicle.getAcceleration(
                    vehicle
                )
                vehicle_measures["position"] = self.sumo.vehicle.getLanePosition(
                    vehicle
                )
                vehicle_measures["type"] = self.sumo.vehicle.getTypeID(vehicle)
                vehicles.append(vehicle_measures)
                if vehicle_measures["wait"] > 0:
                    lane_measures["total_wait"] = (
                        lane_measures["total_wait"] + vehicle_measures["wait"]
                    )
        ...
        if self.last_step_vehicles is None:
            ...
            full_observation["arrivals"] = self.last_step_vehicles.difference(
                all_vehicles
            )
